...
Commentube는 YouTube 영상 링크 하나로 영상 정보를 분석하고, 댓글을 모아 바로 플레이 가능한 게임을 제공합니다. 현재는 `좋아요 배틀`, `대댓글 배틀`, AI 기반 `AI 댓글 찾기`, `타임라인 맞추기`를 지원합니다.
...
대댓글 수가 비슷한 댓글 두 개를 보여주고, 대댓글이 더 많을 것 같은 댓글을 맞히는 게임입니다.

- 댓글 100개 이상 영상에서 시작할 수 있습니다.
- 정답을 고르면 양쪽 댓글의 대댓글 수와 실제 대댓글 목록을 함께 보여줍니다.
- 틀릴 때까지 연속 정답 수를 기록합니다.
...
타임스탬프가 포함된 댓글의 시간을 가리고, 영상을 확인하면서 댓글이 작성한 장면의 범위를 타임라인에서 맞히는 게임입니다. 댓글 50개 이상 영상에서 시작할 수 있으며, 타임스탬프 댓글이 없는 영상은 플레이할 수 없습니다.
...
로컬 개발 모드에서는 `.env.local`의 `RECAPTCHA_BYPASS_LOCAL=true` 설정으로 reCAPTCHA 검증을 건너뜁니다. Vercel Preview와 Production 환경에서는 이 설정이 있어도 우회하지 않고 reCAPTCHA를 검증합니다.
